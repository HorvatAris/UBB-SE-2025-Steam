...
namespace SteamHub.ApiContract.ServiceProxies
{
    public class NewsServiceProxy : ServiceProxy, INewsService
    {
        ...
        public NewsServiceProxy(System.Net.Http.IHttpClientFactory httpClientFactory, string baseUrl = "https://localhost:7241/api/")
            : base(baseUrl)
        {
        ...
        public async Task<bool> DislikePostAsync(int postId, int userId)
        {
            try
            {
                await _httpClient.PostAsync($"News/{userId}/posts/{postId}/dislike", null);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> LikePostAsync(int postId, int userId)
        {
            try
            {
                await _httpClient.PostAsync($"News/{userId}/posts/{postId}/like", null);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<List<Comment>> LoadNextCommentsAsync(int postId, int userId)
        {
            try
            {
                var response = await _httpClient.GetAsync($"News/{userId}/posts/{postId}/comments");
                response.EnsureSuccessStatusCode();
        ...
        public async Task<List<Post>> LoadNextPostsAsync(int pageNumber, string searchedText, int userId)
        {
            try
            {
                var response = await _httpClient.GetAsync($"News/{userId}/posts?page={pageNumber}&search={Uri.EscapeDataString(searchedText ?? string.Empty)}");
                response.EnsureSuccessStatusCode();
        ...
        public async Task<bool> SaveCommentAsync(int postId, string commentContent, int userId)
        {
            try
            {
                ...
                await _httpClient.PostAsync($"News/{userId}/posts/{postId}/comments", content);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> SavePostAsync(string postContent, int userId)
        {
            try
            {
                ...
                await _httpClient.PostAsync($"News/{userId}/posts", content);
                return true;
            }
            catch (Exception ex)
            {
