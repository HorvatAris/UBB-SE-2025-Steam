    ...
    public class Repository : IRepository
    {
        ...
        public void RemoveMessageRequest(Dictionary<string, object> request)
        {
            try
            {
                ...
                // ExecuteDeleteWithAnd doesn't exist, use a SQL query instead
                string query = "DELETE FROM " + Repository.MESSAGE_INVITES_TABLE_NAME + " WHERE ";
                var conditions = request.Select(kvp => kvp.Key + " = " + kvp.Value);
        ...
        public void CancelFriendRequest(int senderUserId, int receiverUserId)
        {
            try
            {
                ...
                // ExecuteNonQuery doesn't exist, execute raw query
                using (var command = new SqlCommand(deleteQuery, databaseConnection.GetConnection()))
                {
                    command.ExecuteNonQuery();
                }
            }
